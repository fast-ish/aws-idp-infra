...
> **Implementation**:
> - **CDK (addons.mustache)**: External Secrets Operator helm chart deployed with IRSA
> - **GitOps (platform/external-secrets/)**:
>   - `cluster-secret-store.yaml` - ClusterSecretStore for AWS Secrets Manager
>   - `external-secrets/backstage-secrets.yaml` - GitHub OAuth, DB credentials
>   - `external-secrets/argocd-secrets.yaml` - GitHub token, admin password
>   - `external-secrets/argo-workflows-secrets.yaml` - ArgoCD creds, Cosign KMS, Slack
>   - `external-secrets/observability-secrets.yaml` - Grafana Cloud credentials
> - **Kyverno Policies (platform/kyverno/policies/secrets/)**:
>   - `require-external-secrets.yaml` - Enforce ExternalSecrets usage in team namespaces
>   - `no-hardcoded-secrets.yaml` - Block hardcoded secrets in deployments
> - **Network Policies**: `platform/network-policies/external-secrets-netpol.yaml`
>   - Least-privilege policies for operator, cert-controller, and webhook
