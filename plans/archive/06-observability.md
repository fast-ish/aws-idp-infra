...
Comprehensive observability using Grafana Cloud k8s-monitoring helm chart for metrics, logs, traces, and profiles with SLO-based alerting.

> **Implementation**:
> - **CDK (addons.mustache)**: Grafana k8s-monitoring helm chart with Alloy collectors
>   - `helm/grafana.mustache` - Full observability stack configuration:
>     - Metrics: Prometheus remote-write to Grafana Cloud (kubelet, cAdvisor, API server, KSM, node-exporter)
>     - Logs: Pod logs, node logs (journald), cluster events via Loki
>     - Traces: OTLP, Zipkin, Jaeger receivers with tail sampling
>     - Profiles: eBPF-based continuous profiling via Pyroscope
>     - Beyla: Zero-code eBPF auto-instrumentation for HTTP/gRPC
>     - Faro: Frontend observability (Real User Monitoring)
>     - OpenCost: Cost allocation integrated with k8s-monitoring
>     - Auto-instrumentation: Java, Node.js, Python, .NET, Go
> - **GitOps (platform/observability/)**: Custom alerting rules only
>   - `alerting/prometheus-rules.yaml` - Infrastructure, pod, deployment, platform, security alerts
>   - `alerting/alertmanager-config.yaml` - Team routing, PagerDuty/Slack integration
>   - `alerting/slo-rules.yaml` - SLO recording rules based on Golden Signals:
>     - Request rate, error rate, latency (P50/P90/P99), saturation metrics
>     - Error budget tracking (30-day rolling)
>     - Multi-window burn rate alerts (fast burn critical, slow burn warning)
>     - Apdex score calculations for user satisfaction
>   - `alerting/cost-alerts.yaml` - OpenCost-based budget and efficiency alerts
> - **Network Policies**: `platform/network-policies/observability-netpol.yaml`
>   - Least-privilege network access for Alloy collectors and OpenCost
