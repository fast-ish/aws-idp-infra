...
// -----------------------------------------------------------------------------
// GitOps Platform Tests
// -----------------------------------------------------------------------------

func (s *TestSuite) testGitOps() {
	s.printHeader("GITOPS PLATFORM")
	ctx := context.Background()

	s.printSection("ApplicationSets")
	appSetGVR := schema.GroupVersionResource{Group: "argoproj.io", Version: "v1alpha1", Resource: "applicationsets"}
	appSets, err := s.dynamicClient.Resource(appSetGVR).Namespace("argocd").List(ctx, metav1.ListOptions{})
	if err == nil {
		ready := 0
		errored := 0
		for _, appSet := range appSets.Items {
			...
			if found {
				...
				for _, c := range conditions {
					cond := c.(map[string]interface{})
					if cond["type"] == "ErrorOccurred" && cond["status"] == "True" {
						hasError = true
						errored++
						s.warn(fmt.Sprintf("ApplicationSet '%s': %s", appSet.GetName(), cond["message"]))
						break
					}
				}
				if !hasError {
		...
		if len(appSets.Items) > 0 {
			s.pass(fmt.Sprintf("ApplicationSets: %d total, %d ready, %d errored", len(appSets.Items), ready, errored))
		} else {
			s.warn("No ApplicationSets found")
		}
	}

	s.printSection("Platform Applications")
	// Check for platform apps generated by platform-apps ApplicationSet
	platformAppPrefixes := []string{
	...
	if err == nil {
		platformApps := 0
		healthyPlatformApps := 0
		for _, app := range apps.Items {
			appName := app.GetName()
			for _, prefix := range platformAppPrefixes {
				if strings.HasPrefix(appName, prefix) {
					...
					if health == "Healthy" && sync == "Synced" {
						...
						s.warn(fmt.Sprintf("%s: health=%s sync=%s", appName, health, sync))
					}
					break
				}
			}
		...
		if platformApps > 0 {
			s.pass(fmt.Sprintf("Platform apps: %d found, %d healthy", platformApps, healthyPlatformApps))
		} else {
			s.warn("No platform apps found - platform-apps ApplicationSet may not be working")
		}
	}

	s.printSection("Team Applications")
	// Check for team apps generated by team-apps ApplicationSet
	teamPrefixes := []string{"backend-", "frontend-", "ml-", "data-", "integrations-", "platform-"}
	teamApps := 0
	healthyTeamApps := 0
	for _, app := range apps.Items {
		appName := app.GetName()
	...
	if teamApps > 0 {
		s.pass(fmt.Sprintf("Team apps: %d found, %d healthy", teamApps, healthyTeamApps))
	} else {
		s.warn("No team apps found - team-apps ApplicationSet may not be working")
	}

	s.printSection("Team Namespaces")
	teamNamespaces := []string{"team-backend", "team-frontend", "team-ml", "team-data", "team-integrations", "team-platform"}
	for _, ns := range teamNamespaces {
		_, err := s.clientset.CoreV1().Namespaces().Get(ctx, ns, metav1.GetOptions{})
		if err == nil {
			// Check for ResourceQuota
			quotas, _ := s.clientset.CoreV1().ResourceQuotas(ns).List(ctx, metav1.ListOptions{})
			if len(quotas.Items) > 0 {
				s.pass(fmt.Sprintf("%s: exists with %d quota(s)", ns, len(quotas.Items)))
			} else {
				s.warn(fmt.Sprintf("%s: exists but no ResourceQuota", ns))
			}
		} else {
			s.fail(fmt.Sprintf("%s: not found", ns))
		}
	}
}
